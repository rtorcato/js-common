...
- **42 subpath modules**, one concern per module — see the [Module overview](./modules/overview.md).
...
| Collections | `arrays`, `objects`, `maps` | `unique`, `chunk`, `mergeMaps`, `deepMerge`, `pick`, `omit` |
| Async | `promises`, `sleep`, `abortController`, `functions` | `withTimeout`, `sleep`, `sleepRandom`, `debounce`, `throttle` |
| Validation | `validation`, `emails`, `url`, `uuid` | `isValidEmail`, `isValidUrl`, `getUUIDv7` |
