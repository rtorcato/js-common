...
| `isEmail` | **Deprecated.** Use `isValidEmail` from `@rtorcato/js-common/emails`. Checks if a string is a valid email address (simple regex). |
...
| `isUrl` | **Deprecated.** Use `isValidUrl` from `@rtorcato/js-common/url`. Checks if a string is a valid URL. |
